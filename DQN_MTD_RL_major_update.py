...
class DQNAgent:
    def __init__(self, state_dim: int, action_dim: int, numOfZ: int, hidden_size: int = 256, sequence_length: int = 10, feature_dim: int = 13):
        ...
        self.random_action_prob = 0.2  # Probability of taking completely random action
...
class IEEE14BusPowerSystemEnv(gym.Env):
    ...
    def step(self, action):
        ...
        try:
            ...
            start_pointer = self.attack_index_pointer
            
            while len(processed_attacks) < len(self.attack_indices):
                self.current_attack = self.attack_indices[self.attack_index_pointer]
                
                if self.current_attack in processed_attacks:
                    break
                ...
                processed_attacks.add(self.current_attack)
                ...
                self.attack_index_pointer = (self.attack_index_pointer + 1) % len(self.attack_indices)
                if self.attack_index_pointer == start_pointer:
                    break
            ...
            info = {
                ...
                'successful_detection': self.successfulDetectionCount,
                'successful_detection_count': self.successfulDetectionCount_avg,
                'processed_attacks': len(processed_attacks),
