...
class DataProcessor:
    ...
    def apply_id_mapping(self, Z_mat: np.ndarray, IDbank: list) -> np.ndarray:
        ...
        selectedIDs, deceptiveIDs = IDbank[1]  # Using first level of ID bank
...
class DQNAgent:
    def __init__(self, state_dim: int, action_dim: int, numOfZ: int, hidden_size: int = 256, 
                sequence_length: int = 10, feature_dim: int = 13):
        ...
        self.steps = 0
        self.target_update_freq = 500  # How often to update target network
        self.train_freq = 10           # How often to train

        # Exploration parameters
        self.epsilon = 1.0             # Start with full exploration
        self.epsilon_min = 0.05        # Minimum exploration rate
        self.epsilon_decay = 0.995     # How quickly to reduce exploration
        self.exploration_phase = 5000   # Initial exploration steps
        
        # Possible deception amounts (strategic values)
        self.possible_deceptions = np.array([20, 35, 50, 65, 80])
        ...
        self.success_history = deque(maxlen=5)   # Track last 5 rewards
        self.consistency_weight = 0.3            # Weight for consistency reward
        self.consistency_threshold = 10.0            # Maximum deviation for consistency
        self.success_threshold = 0.0                 # Minimum reward for success
    ...
    def build_model(self, hidden_size):
        ...
        model = tf.keras.Sequential([
            ...
            layers.Bidirectional(layers.LSTM(hidden_size, activation='relu', return_sequences=True)),
            layers.Dropout(0.2),
            layers.Bidirectional(layers.LSTM(hidden_size, activation='relu', return_sequences=True)),
            layers.Dropout(0.2),
            layers.Bidirectional(layers.LSTM(hidden_size, activation='relu', return_sequences=False)),
            layers.Dropout(0.2),
    ...
    def calculate_consistency_bonus(self, current_deception):
        """Calculate bonus reward for consistent actions"""
        try:
            if len(self.action_history) < 2:
                return 0.0

            # Get recent deception amounts
            recent_deceptions = np.array([float(action['deception_amount'][0]) 
                                        for action in self.action_history])
            recent_rewards = np.array(list(self.success_history))

            # Ensure we have matching lengths
            min_length = min(len(recent_deceptions), len(recent_rewards))
            if min_length < 2:
                return 0.0

            recent_deceptions = recent_deceptions[-min_length:]
            recent_rewards = recent_rewards[-min_length:]

            # Calculate weighted average of recent successful actions
            if len(recent_rewards) > 0:
                # Normalize rewards to [0,1] range
                normalized_rewards = recent_rewards - np.min(recent_rewards)
                reward_sum = np.sum(normalized_rewards)
                
                if reward_sum > 0:
                    normalized_rewards = normalized_rewards / reward_sum
                    # Calculate weighted average
                    weighted_deception = np.sum(recent_deceptions * normalized_rewards)
                else:
                    weighted_deception = np.mean(recent_deceptions)
            else:
                weighted_deception = np.mean(recent_deceptions)

            # Calculate consistency bonus
            deviation = abs(current_deception - weighted_deception)
            consistency_bonus = max(0, 1.0 - (deviation / self.consistency_threshold))

            print(f"Consistency Calculation:")
            print(f"Current Deception: {current_deception}")
            print(f"Weighted Deception: {weighted_deception}")
            print(f"Deviation: {deviation}")
            print(f"Consistency Bonus: {consistency_bonus * self.consistency_weight}")

            return consistency_bonus * self.consistency_weight

        except Exception as e:
            print(f"Error in calculate_consistency_bonus: {str(e)}")
            return 0.0
    ...
    def act(self, state, training=True):
        ...
        try:
            ...
            # Update epsilon
            if self.steps % 100 == 0 and training:
                self.epsilon = max(
                    self.epsilon_min,
            ...
            if training and np.random.rand() <= self.epsilon:
                # Exploration with history influence
                if len(self.action_history) > 0 and np.random.rand() < 0.3:
                    # 30% chance to stay close to previous action if it was successful
                    last_action = self.action_history[-1]
                    last_reward = self.success_history[-1] if len(self.success_history) > 0 else 0
                    
                    if last_reward > self.success_threshold:
                        # Stay close to last successful action
                        base_deception = last_action['deception_amount'][0]
                        noise = np.random.normal(0, 5.0)
                        deception_amount = self._get_strategic_deception(base_deception + noise)
                    else:
                        # Regular exploration
                        deception_amount = np.random.choice(self.possible_deceptions)
                else:
                    # Regular exploration
                    deception_amount = np.random.choice(self.possible_deceptions)
            else:
                ...
                deception_amount = self._get_strategic_deception(q_values[0] * 80)

            # Get sensor selection
            sensor_selection = self._get_sensor_selection(
                q_values if 'q_values' in locals() else None
            )
    ...
    def _get_strategic_deception(self, value):
        """Round to nearest strategic deception value"""
        value = np.clip(value, 20, 80)
        return self.possible_deceptions[
            np.abs(self.possible_deceptions - value).argmin()
        ]

    def _get_sensor_selection(self, q_values=None):
        """Get sensor selection based on Q-values or random"""
        if q_values is None or np.random.rand() <= self.epsilon:
            return np.random.randint(2, size=self.numOfZ)
        return (q_values[1:self.numOfZ+1] > 0.5).astype(int)

    def remember(self, state, action, reward, next_state, done):
        """Store experience in memory with consistency bonus"""
        try:
            # Get current deception amount
            deception_amount = float(action['deception_amount'].item())
            
            # Store the original reward in success history before calculating bonus
            self.success_history.append(reward)
            
            # Calculate consistency bonus
            consistency_bonus = self.calculate_consistency_bonus(deception_amount)
            
            # Add consistency bonus to reward
            adjusted_reward = reward + consistency_bonus
            ...
            # Store action in history after successful processing
            self.action_history.append({
                'deception_amount': np.array([deception_amount]),
                'sensor_selection': sensor_selection
            })
            
            # Calculate priority based on adjusted reward
            priority = abs(adjusted_reward) + 1.0
            
            # Store experience with priority
            self.memory.append({
                'state': state,
                'action': np.concatenate(([deception_amount], sensor_selection)),
            ...
            print(f"\nMemory Storage:")
            print(f"Original Reward: {reward:.2f}")
            ...
            print(f"Success History Length: {len(self.success_history)}")
    ...
    def replay(self):
        ...
        try:
            ...
            experiences = sorted(self.memory, key=lambda x: x['priority'], reverse=True)
            minibatch = experiences[:self.batch_size]
            ...
            dones = np.array([exp['done'] for exp in minibatch])
    ...
    def get_default_action(self):
        """Return default action when errors occur"""
        return {
    ...
    def get_action_statistics(self):
        ...
        recent_deceptions = [action['deception_amount'][0] for action in self.action_history]
        return {
            'mean_deception': np.mean(recent_deceptions),
            'std_deception': np.std(recent_deceptions),
            'min_deception': min(recent_deceptions),
            'max_deception': max(recent_deceptions),
            'recent_rewards': list(self.success_history),
...
class IEEE14BusPowerSystemEnv(gym.Env):
    ...
    def _calculate_reward(self, state_deviation, successful_attack, successful_detection, successful_detection_count, deception_amount):
        ...
        reward -= state_deviation * 1
        
        # Penalize successful attacks
        if successful_attack:
            reward -= 20*successful_attack
        
        # Reward detection
        if successful_detection:
            reward += 30*successful_detection
        
        # Penalize excessive deception
        reward -= (deception_amount) *1
